...
object CypherQuery {
  ...
  private def apply(q: Query, distinct: Boolean): CypherQuery =
    q match {
  ...
  private def apply(q: Query, select: List[Ast], distinct: Boolean): CypherQuery =
    q match {
  ...
  private def apply(q: Query, limit: Option[Ast], select: List[Ast], distinct: Boolean): CypherQuery =
    q match {
  ...
  private def apply(q: Query, orderBy: List[OrderByCriteria], limit: Option[Ast], select: List[Ast], distinct: Boolean): CypherQuery =
    q match {
  ...
  private def apply(q: Query, filter: Option[Ast], orderBy: List[OrderByCriteria], limit: Option[Ast], select: List[Ast], distinct: Boolean): CypherQuery =
    q match {
      case q: Entity =>
        new CypherQuery(q, "label")
      case (_: FlatMap) =>
        fail(s"Cypher doesn't support flatMap.")
      case (_: Union) | (_: UnionAll) =>
        fail(s"Cypher doesn't support union/unionAll.")
      case Join(joinType, _, _, _, _, _) =>
        fail(s"Cypher doesn't support $joinType.")
      case _: GroupBy =>
        fail(s"Cypher doesn't support groupBy.")
      case q =>
